...
pub struct MVerb {
    ...
    sample_rate: f64,
    damping_frequency: f64,
    density1: f64,
    density2: f64,
    bandwidth_frequency: f64,
    pre_delay_time: f64,
    pub decay: f64,
    // gain: f64,  // Todo: should be removed?
    pub mix: f64,
    pub early_mix: f64,
    pub size: f64,
    mix_smooth: f64,
    early_late_smooth: f64,
    bandwidth_smooth: f64,
    damping_smooth: f64,
    predelay_smooth: f64,
    size_smooth: f64,
    density_smooth: f64,
    decay_smooth: f64,
    previous_left_tank: f64,
    previous_right_tank: f64,
    control_rate: usize,
...
impl MVerb {
    ...
    pub fn process(&mut self, input: (f64, f64)) -> (f64, f64) {
        ...
        let over_sample_frames: f64 = 1. / sample_frames;
...
impl<const MAX_LENGTH: usize> AllPass<MAX_LENGTH> {
    pub fn operator(&mut self, input: f64) -> f64 {
...
impl<const MAX_LENGTH: usize> StaticAllPassFourTap<MAX_LENGTH> {
    fn operator(&mut self, input: f64) -> f64 {
    ...
    fn get_index(&self, index: usize) -> f64 {
        match index {
...
impl<const MAX_LENGTH: usize> StaticDelayLineEightTap<MAX_LENGTH> {
    fn operator(&mut self, input: f64) -> f64 {
        let output: f64 = self.buffer[self.index1];
        self.buffer[self.index1] = input;
        self.index1 += 1;
...
impl<const OVER_SAMPLE_COUNT: usize> LowPassFilter<OVER_SAMPLE_COUNT> {
    ...
    fn set_sample_rate(&mut self, sample_rate: f64) {
        self.sample_rate = sample_rate * OVER_SAMPLE_COUNT as f64;
    ...
    fn update_coefficient(&mut self) {
        self.f = 2. * (std::f64::consts::PI * self.frequency / self.sample_rate).sin();
    }
}
